...
def use_scale(n, cycle_number, SAMPLE, rtde_c, rtde_r, rtde_io, balance, remote, photo_directory):
    ...
    while measurement_count <= 3:
        ...
        measured_weight = float(measure(balance=balance, remote=remote, raspberry_pi_ip="192.168.8.151"))
...
def photo_stand(n, cycle_number, rtde_c, rtde_r, rtde_io, photo_directory):
    ...
    cv2.imwrite(photo_directory + "/Sample_" + str(n) + "_cycle_" + str(Ncyc) +"_front.png", photo)
    ...
    photo = environment.take_photo(2)
    cv2.imwrite(photo_directory + "/Sample_" + str(n) + "_cycle_" + str(Ncyc) +"_side.png", photo)
