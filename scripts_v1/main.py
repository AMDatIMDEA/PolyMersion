...
def main():
    ...
    for i in range(1, cycles + 1):
        first = (i - 1) * 11
        Groups[i] = np.arange(first + 1, first + 11)

    selected_samples = {}
    for i in range(1, cycles + 1):
        selected_samples[i] = Groups[i][:samples_per_group] 
    ...
    remote, balance = setup_remote_scale(scale_access_remote)
    ...
    while cycle_number < cycles + 1:
        # Time delay management
        minutes = int(time.strftime('%M', time.localtime())) + minutes_delay
        hours = int(time.strftime('%H', time.localtime())) + hours_delay
        if minutes >= 60:
            minutes -= 60
            minutes += 1
        ...
        if len(str(hours)) == 1:
            H = '0' + str(hours)
        time_delay = str(hours) + str(minutes)
        ...
        for n in SEQUENCE:
            ...
            P0[0] = PX + (xtemp * math.cos(angle_deviation) - ytemp * math.sin(angle_deviation))  # Rot transformation X
            if n > 121:
                P0[0] += 0.003
            P0[1] = PY + (ytemp * math.cos(angle_deviation) + xtemp * math.sin(angle_deviation))  # Rot transformation Y
            ...
            environment.photo_stand(n, cycle_number, rtde_c, rtde_r, rtde_io, photo_dir)
            
            # Measure the weight of the sample on the scale
            rtde_c.moveL(scale_position, 3, 1)
            environment.use_scale(n, cycle_number, SAMPLE, rtde_c, rtde_r, rtde_io, balance, remote, photo_dir)
            ...
            while len(temp.split()) < 5:  # Ensure temperature is valid
                temperature = environment.arduino(b'TEMPERATURE')
                time.sleep(1)
            ...
            while True:
                if choice == "1":
                    ...
                    degradation.replace_sample_out(rtde_c, rtde_r, rtde_io, PD, Pini)
    ...
    # STEP 5: GENERATE GRAPH AND SAVE IMAGE
    df = pd.read_csv(csv_file)
    average_columns = [col for col in df.columns if 'Average' in col]
    samples = df['Sample'].unique()
    cycles_1 = [int(col.split()[1]) for col in average_columns]
    
    average_weights = {sample: [] for sample in samples}
    for sample in samples:
        for col in average_columns:
            average = df[df['Sample'] == sample][col].values[0]
            average_weights[sample].append(average)

    plt.figure(figsize=(14, 8))
    for sample, averages in average_weights.items():
        plt.plot(cycles_1, averages, marker='o', label=f'Sample {sample}')
    
    plt.title('Average Weight per Cycle', fontsize=16)
    plt.xlabel(f'Cycle Number, {hours_delay} Hr {minutes_delay} Min Delay', fontsize=14)
